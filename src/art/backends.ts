import type { ArtBackendResult, ArtFit } from "../types.ts";
import type { ReloadOpts } from "./ghostty-reload.ts";
import {
...
import { reloadGhosttyConfig } from "./ghostty-reload.ts";
import {
  DEFAULT_ART_FIT,
  DEFAULT_ART_OPACITY,
  cssBackgroundSize,
...
/**
 * Option bag shared by every `art` backend.
 *
 * These functions all take the same shape: paths + an injectable filesystem
 * and process layer so tests can run them against a temp HOME without
 * touching the real machine. The index signature keeps per-backend extras
 * (brightness, stretch modes, reload hooks) legal without one interface per
 * terminal.
 */
export interface BackendOpts {
  ...
  kill?: ReloadOpts["kill"];
  findPids?: ReloadOpts["findPids"];
  signalPids?: ReloadOpts["signalPids"];
  appleScriptReload?: ReloadOpts["appleScriptReload"];
  reloadHint?: string | null;
  psOutput?: ReloadOpts["psOutput"];
  [key: string]: unknown;
}
...
function hasGhosttyArtMarker(text: string) {
  return (
 ...
 * @returns {string}
 */
export function resolveGhosttyConfigPath(opts: BackendOpts = {}) {
 ...
 * @returns {boolean}
 */
export function isAlquimiaGhosttyImagePath(
  value: string,
  artPath?: string,
): boolean {
 ...
 * @returns {string}
 */
export function clearOrphanAlquimiaGhosttyKeys(
  content: string,
  { artPath = "" }: BackendOpts = {},
) {
 ...
 * @returns {string}
 */
export function clearGhosttyArtFromConfig(content: string, opts: BackendOpts = {}) {
...
export function patchGhosttyConfigContent(
  content: string,
  imagePath: string,
  { opacity = GHOSTTY_DEFAULT_OPACITY, fit = DEFAULT_ART_FIT }: BackendOpts = {},
) {
...
export function ghosttyReloadHint(osPlatform: string = platform()) {
...
function ghosttyPostWriteReload(opts: BackendOpts = {}): {
  reloaded: boolean;
  reloadMethod: string | null;
  reloadHint?: string;
  needsReload: boolean;
} {
  ...
  if (reload.ok) {
    return {
      reloaded: true,
      reloadMethod: reload.method ?? null,
      needsReload: false,
    };
  }
  return {
 ...
 * @param {object} [opts]
 */
export function setGhosttyBackground(
  imagePath: string,
  opts: BackendOpts = {},
): ArtBackendResult {
  const exists = opts.existsSync ?? fsExistsSync;
 ...
 * @param {object} [opts]
 */
export function clearGhosttyBackground(opts: BackendOpts = {}): ArtBackendResult {
  const exists = opts.existsSync ?? fsExistsSync;
  const read = opts.readFileSync ?? fsReadFileSync;
  const candidates = ghosttyConfigCandidates(opts);
 ...
 * @returns {string}
 */
export function resolveWeztermConfigPath(opts: BackendOpts = {}) {
  const exists = opts.existsSync ?? fsExistsSync;
  const read = opts.readFileSync ?? fsReadFileSync;
...
export function patchWeztermConfigContent(
  content: string,
  imagePath: string,
  { brightness = WEZTERM_BRIGHTNESS, fit = DEFAULT_ART_FIT }: BackendOpts = {},
) {
 ...
 * @returns {string}
 */
export function clearWeztermArtFromConfig(content: string) {
 ...
 * @param {object} [opts]
 */
export function setWeztermBackground(
  imagePath: string,
  opts: BackendOpts = {},
): ArtBackendResult {
  const exists = opts.existsSync ?? fsExistsSync;
 ...
 * @param {object} [opts]
 */
export function clearWeztermBackground(opts: BackendOpts = {}): ArtBackendResult {
  const exists = opts.existsSync ?? fsExistsSync;
  const read = opts.readFileSync ?? fsReadFileSync;
  const candidates = weztermConfigCandidates(opts);
  const targets = candidates.filter((p: string) => {
 ...
 * @returns {string}
 */
export function resolveContourConfigPath(opts: BackendOpts = {}) {
  const exists = opts.existsSync ?? fsExistsSync;
  const read = opts.readFileSync ?? fsReadFileSync;
...
export function patchContourConfigContent(
  content: string,
  imagePath: string,
  { opacity = CONTOUR_OPACITY }: BackendOpts = {},
) {
  const body = [
    "    background_image:",
 ...
 * @returns {string}
 */
export function clearContourArtFromConfig(content: string) {
 ...
 * @param {object} [opts]
 */
export function setContourBackground(
  imagePath: string,
  opts: BackendOpts = {},
): ArtBackendResult {
  const exists = opts.existsSync ?? fsExistsSync;
 ...
 * @param {object} [opts]
 */
export function clearContourBackground(opts: BackendOpts = {}): ArtBackendResult {
...
function escapeYamlSingle(s: string) {
 ...
 * @returns {string}
 */
export function resolveHyperConfigPath(opts: BackendOpts = {}) {
  const exists = opts.existsSync ?? fsExistsSync;
  const read = opts.readFileSync ?? fsReadFileSync;
...
export function patchHyperConfigContent(
  content: string,
  imagePath: string,
  { fit = DEFAULT_ART_FIT }: BackendOpts = {},
) {
 ...
 * @returns {string}
 */
export function clearHyperArtFromConfig(content: string) {
 ...
 * @param {object} [opts]
 */
export function setHyperBackground(
  imagePath: string,
  opts: BackendOpts = {},
): ArtBackendResult {
  const exists = opts.existsSync ?? fsExistsSync;
 ...
 * @param {object} [opts]
 */
export function clearHyperBackground(opts: BackendOpts = {}): ArtBackendResult {
  return clearMarkedConfigs({
...
function pathToFileUrl(absPath: string) {
...
export function patchTabbyConfigContent(
  content: string,
  imagePath: string,
  { opacity = TABBY_BG_OPACITY, fit = DEFAULT_ART_FIT }: BackendOpts = {},
) {
  const fileUrl = pathToFileUrl(imagePath);
  const size = cssBackgroundSize(fit || DEFAULT_ART_FIT);
  const css = [
 ...
 * @returns {string}
 */
export function clearTabbyArtFromConfig(content: string) {
  const text = content == null ? "" : String(content);
  return text
    .replace(
      new RegExp(
 ...
 * @param {object} [opts]
 */
export function setTabbyBackground(
  imagePath: string,
  opts: BackendOpts = {},
): ArtBackendResult {
  const exists = opts.existsSync ?? fsExistsSync;
 ...
 * @param {object} [opts]
 */
export function clearTabbyBackground(opts: BackendOpts = {}): ArtBackendResult {
  return clearMarkedConfigs({
...
export function patchWindowsTerminalSettings(
  content: string,
  imagePathWindows: string,
  { opacity = WT_OPACITY, fit = DEFAULT_ART_FIT }: BackendOpts = {},
) {
 ...
 * @returns {string}
 */
export function clearWindowsTerminalSettings(content: string) {
  let text = content == null ? "" : String(content);
  text = text.replace(
    /\/\*\s*>>> alquimia-art >>>\s*\*\/[\s\S]*?\/\*\s*<<< alquimia-art <<<\s*\*\//g,
    ""
 ...
 * @returns {string}
 */
export function toWindowsPathIfWsl(absPath: string) {
 ...
 * @param {object} [opts]
 */
export function setWindowsTerminalBackground(
  imagePath: string,
  opts: BackendOpts = {},
): ArtBackendResult {
  const exists = opts.existsSync ?? fsExistsSync;
 ...
 * @param {object} [opts]
 */
export function clearWindowsTerminalBackground(opts: BackendOpts = {}): ArtBackendResult {
  const exists = opts.existsSync ?? fsExistsSync;
  const read = opts.readFileSync ?? fsReadFileSync;
  const candidates = windowsTerminalSettingsCandidates(opts);
 ...
 */
export function setTilixBackground(
  imagePath: string,
  opts: BackendOpts = {},
): ArtBackendResult {
 ...
 * @param {{ spawnSync?: typeof spawnSync }} [opts]
 */
export function clearTilixBackground(opts: BackendOpts = {}): ArtBackendResult {
  const run = opts.spawnSync ?? spawnSync;
  const set = run(
    "gsettings",
 ...
 * @param {{ spawnSync?: typeof spawnSync }} [opts]
 */
export function setTerminologyBackground(
...
export function clearTerminologyBackground(opts: BackendOpts = {}): ArtBackendResult {
 ...
 */
export function writeItermBackground(path: string | null) {
 ...
 */
export interface KittyRemoteResult {
  ok: boolean;
  stderr: string;
  code: number;
}

export function runKittyRemote(args: string[]): Promise<KittyRemoteResult> {
  return new Promise<KittyRemoteResult>((resolve) => {
...
export async function setKittyBackground(
  path: string | null,
