...
import { plainOf } from "../src/ui/style.ts";
...
import { GHOSTTY_BLOCK_BEGIN, GHOSTTY_BLOCK_END } from "../src/art.ts";
...
describe("collectDoctorReport / formatDoctorReport", () => {
  it("builds a report with expected keys", () => {
    ...
    try {
      ...
      // formatDoctorReport now returns OpenTUI StyledText lines.
      const text = formatDoctorReport(report).map(plainOf).join("\n");
